...
from src.layers.dc_layers import DropConnectConv
...
def compute_average_flops_cost(self):
  ...
  for module in self.modules():
    ...
    if isinstance(module, DropConnectConv):
...
def get_per_layer_flops(model, trainloader):
  model = add_flops_counting_methods(model)
  model.start_flops_count()
  loader = iter(trainloader)
  data = loader.next()
  batch = torch.autograd.Variable(data[0])
  _ = model(batch)  # works now
  count = 1
  Gflops = []
  mid_layers = []
  first_layers = []
  for layer in model.modules():
    if isinstance(layer, DropConnectConv):
      flops = layer.__flops__ / 1e9 / 2
      logger.info('Layer {} Flops {:.4f}'.format(count, flops))
      Gflops.append(flops)
      count += 1
      if layer.needs_drop:
        mid_layers.append(flops)
      else:
        first_layers.append(flops)

  logger.info('Total Gflops 25 samples {:.4f}'.format(sum(Gflops) * 25))
  logger.info('MCDC Gflops 25 samples {:.4f}'.format(sum(first_layers) + (sum(mid_layers) * 25)))
